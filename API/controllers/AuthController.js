...
export const Login = async (req, res, next) => {
  if (req.body.email === "" || req.body.password === "")
    return next(createError(404, "Please enter your email and Password"));
  try {
    const user = await User.findOne({ email: req.body.email });
    ...
    if (!user.isVerified) {
      if (user.verificationExpires.getTime() < Date.now()) {
        try {
          ...
          if (delUser)
            return next(
              createError(400, "Email failed to verify, account deleted")
            );
        } catch (error) {
          return next(error);
        }
    ...
    if (user.isDeactivated)
      return next(createError(400, "Account deactivated, contact admin"));

    const passwordCorrect = await bcrypt.compare(
      req.body.password,
      user.password
    );
    if (!passwordCorrect) return next(createError(403, "Incorrect password"));
