...
import {
  ...
  sendNotificationSwapRecipient,
  sendNotificationSwapSender,
...
export const CreateShiftSwapRequest = async (req, res, next) => {
  try {
    ...
    let existingRequest = await RequestSwap.find({
      status: "pending",
      date,
      requestingShiftId,
      requesterShiftId,
    ...
    if (requesterName === "Unknown Unknown")
      return next(createError(400, "Invalid request"));

    const requestShift = await Shift.findById(requestingShiftId);
    if (requestShift) {
      const isRequestedByIncluded =
        requestShift.assignedEmployees.includes(requestedBy);
      const isRequestedToIncluded =
        requestShift.assignedEmployees.includes(requestedTo);

      if (isRequestedByIncluded && isRequestedToIncluded) {
        return next(
          createError(
            400,
            "You cannot request a swap when both employees are already on the same schedule."
          )
        );
      }
      ...
      if (!isRequestedByIncluded && !isRequestedToIncluded) {
        return next(
          createError(
            400,
            "Both employees are WFH on this shift. No swap is needed."
          )
        );
      }
    }
    ...
    if (offerShift) {
      const isRequestedByIncluded =
        offerShift.assignedEmployees.includes(requestedBy);
      const isRequestedToIncluded =
        offerShift.assignedEmployees.includes(requestedTo);

      if (isRequestedByIncluded && isRequestedToIncluded) {
        return next(
          createError(
            400,
            "Your offered shift already includes both employees. You cannot send a request for the same schedule."
          )
        );
      }
      ...
      const htmlContent = `
                        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                            ...
                            <li><strong>Requester:</strong> ${
                              requester.firstname
                            } ${requester.lastname} </li>
                            <li><strong>Recipient:</strong> ${
                              recipient.firstname
                            } ${recipient.lastname} </li>
                            ...
                            <li><strong>Requester Message:</strong> ${
    ...
    res.status(200).json({
      message: "Request sent successfully and email notifications sent",
      requestSwapShift: existingRequest,
    });
  } catch (error) {
    next(error);
  }
...
export const AcceptSwapRequestShift = async (req, res, next) => {
  try {
    ...
    if (!existingRequestShift || !existingOfferShift) {
      return res
        .status(404)
        .json({ message: "Invalid request. Shift not found." });
    }
    ...
    if (requestAccept) {
                ...
                <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    ...
                    <ul>
                        ...
                        <li><strong>Recipient Message:</strong> ${
                          recipientMessage || "No message provided"
                        }</li>
                    </ul>
                    <p>Thank you!</p>
                </div>
      ...
      return res.status(200).json({
        message:
          "Request accepted! Shift updated successfully and email notifications sent.",
        data: requestAccept,
      });
    } else {
...
export const DeclineSwapRequestShift = async (req, res, next) => {
  try {
    ...
    if (requestDecline) {
      ...
      return res.status(200).json({
        message:
          "Request declined! Shift updated successfully and email notifications sent.",
        data: requestDecline,
      });
    } else {
      return res.status(400).json({ message: "Request Invalid" });
...
export const ApprovedSwapRequestAdmin = async (req, res, next) => {
  try {
    ...
    const recipientName =
      request.requestedTo.firstname + " " + request.requestedTo.lastname;
    const RequestshiftDate = new Date(
      existingRequestShift.date
    ).toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    const OffershiftDate = new Date(existingOfferShift.date).toLocaleDateString(
      "en-US",
      {
        year: "numeric",
        month: "long",
    ...
    if (RecipientST === "on-site") {
      ...
      return res.status(200).json({
        message: "Both employees are on WFH schedule; no update needed.",
      });
    }
