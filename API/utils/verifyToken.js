...
export const verifyToken = (req, res, next) => {
  let token =
    req.cookies.access_token || req.headers.authorization?.split(" ")[1]; // Support both cookie and header
  if (!token) {
...
export const logTokenRequest = (req, res, next) => {
  ...
  const userId =
    req.headers["user-id"] || req.headers["admin-id"] || "anonymous"; // Default to 'anonymous' if no ID is provided

  // Determine if the user is an admin
  const isAdmin = req.headers["admin-id"] ? true : false;

  // Create the appropriate logger
  const userLogger = isAdmin
    ? createAdminLogger(userId)
