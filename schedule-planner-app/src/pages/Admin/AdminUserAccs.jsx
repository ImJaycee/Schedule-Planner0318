...
const UserAccs = () => {
  ...
  const [formData, setFormData] = useState({ firstname: "", lastname: "", email: "", password: "", isAdmin: false, isVerified: true, department: department });
  ...
  const fetchUsers = async () => {
    try {
      const { data } = await axios.get(`http://localhost:4000/api/user-manage/users`, {
        ...
        params: { department } // Ensure department is sent properly
      });
      setUsers(data);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      fetchUsers();
      setFormData({ firstname: "", lastname: "", email: "", password: "", isAdmin: false, isVerified: true, department: department });
    } catch (error) {
