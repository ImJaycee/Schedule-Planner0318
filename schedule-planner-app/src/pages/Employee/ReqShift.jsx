...
const RequestShift = () => {
  ...
  const handleDateClick = (info) => {
    ...
    const shiftsForDate = events.filter(
      (event) =>
  ...
  useEffect(() => {
    const formattedEvents = data.map((shift) => {
      ...
      const assignedEmployees = shift.assignedEmployees
        .filter((emp) => emp._id === requestedBy)
        .map((emp) => emp.firstname);

      setAssigned(
        assignedEmployees.length > 0 && shift.shiftType === "on-site"
          ? "On-site"
          : "WFH"
      );
      ...
      return {
        ...
        color:
          shift.shiftType === "on-site" && assignedEmployees.length > 0
            ? "green"
            : "gray",
        allDay: true,
        extendedProps: {
